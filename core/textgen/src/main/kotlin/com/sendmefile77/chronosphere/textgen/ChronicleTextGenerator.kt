...
class ChronicleTextGenerator {
    fun describe(event: SimulationEvent): String = when (event.code) {
        ...
        "PLAYER_EVOLUTION_DIVERGENCE" -> "За втручанням гравця в ${event.facts["settlement"] ?: "поселенні"} від лінії ${event.facts["parent"] ?: "населення"} відокремилася прискорено змінена лінія ${event.facts["lineage"] ?: "нова лінія"}."
        "PLAYER_STRUCTURAL_MUTATION" -> "Гравець спричинив структурний еволюційний зсув у ${event.facts["settlement"] ?: "поселенні"}: ${event.facts["lineage"] ?: "нова лінія"} отримала ${event.facts["bodyPlan"] ?: "новий план тіла"}."
        "PLAYER_HYBRIDIZATION" -> "За рішенням гравця в ${event.facts["settlement"] ?: "поселенні"} створено гібридну лінію ${event.facts["lineage"] ?: "населення"} з ліній ${event.facts["primary"] ?: "першої"} та ${event.facts["secondary"] ?: "другої"}."
